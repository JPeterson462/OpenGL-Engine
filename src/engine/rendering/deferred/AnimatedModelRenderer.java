...
public class AnimatedModelRenderer {
	...
	public void bind(boolean normalMapped, Engine engine, Camera camera, Vector3f skyColor, Vector4f plane, float ambientLightFactor) {
		if (normalMapped) {
			bind(normalShader, engine, camera, skyColor, plane, ambientLightFactor);
		} else {
			bind(regularShader, engine, camera, skyColor, plane, ambientLightFactor);
		}
	}
	
	private void bind(Shader shader, Engine engine, Camera camera, Vector3f skyColor, Vector4f plane, float ambientLightFactor) {
		this.shader = shader;
